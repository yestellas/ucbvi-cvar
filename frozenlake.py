...
def invent_opt(P, R, H, sigma, Sigma):
    ...
    np.save("./params/frozenlake_10_pi_star_1", pi_star)
...
class FrozenLake(FiniteMDP):
    ...
    def __init__(self, slip_prob, alpha, K, I, T, desc=None, map_name="map2"):
    ...
    def reset(self):
        ...
        self.state = 39*(self.I*2 +1) + self.I + self.alpha
            ...
            for i in range(-self.I, self.I+1):
                for a in range(4):
                    ...
                    for ni in range (-self.I, self.I+1):
                        ...
                        #print(ss, s, i, self.R[s][a], bi, bni, bnni)
                        if ((bnni - bi) > self.R[s][a]) and ((bni - bi) <= self.R[s][a]):
...
if __name__ == '__main__':
    ...
    env = FrozenLake(slip_prob=prob, alpha = 1, I = i, K = k, T = t)
