...
from agents.utils import backward_induction_in_place
...
class UCBVIAgent(AgentWithSimplePolicy):
    ...
    def reset(self, **kwargs):
        ...
        self.P_hat = np.zeros(shape_hsas) * 1.0 / S
    ...
    def _compute_bonus(self, n, hh):
        ...
        if self.bonus_type == "simplified_bernstein":
            ...
            return bonus
        elif self.bonus_type == "simplified_bernstein_2":
            bonus = 6e-3 * self.horizon * np.sqrt(1.0 / n)
            bonus = min(bonus, self.v_max[hh])
            return bonus
        else:
            bonus = 0 * self.horizon * np.sqrt(1.0 / n)
            bonus = min(bonus, self.v_max[hh])
            return bonus
            #raise ValueError(
    ...
    def _run_episode(self):
        ...
        #print("run episode", self.episode)
        for hh in range(self.horizon):
        ...
        if not self.real_time_dp:
            if self.stage_dependent:
                #print("backward_induction, stage dependent")
                backward_induction_sd(
                    self.Q,
                    self.V,
                ...
                #print("backward_induction, stage independent")
                backward_induction_reward_sd(
                    self.Q,
                    self.V,
                    self.R_hat + self.B_sa,
    ...
    def _eval(self):
        ...
        # writer
        if self.writer is not None:
            self.writer.add_scalar("value", value, self.episode)
        self.value[self.episode-1] = value

        # return sum of rewards collected in the episode
        return value

    def fit(self, budget: int, seed : int, **kwargs):
        del kwargs
